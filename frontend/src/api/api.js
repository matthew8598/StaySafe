...
function loadAllThresholds() {
  try { return JSON.parse(localStorage.getItem(THRESH_KEY)) || {}; }
  catch { return {}; }
}
...
function generateReadings(sensorType, count, intervalMinutes = 1) {
  ...
  for (let i = count; i >= 1; i--) {
    if (sensorType === 'light') {
      ...
      v = Math.random() < 0.06 ? parseFloat((Math.random() * 90 + 8).toFixed(1)) : 0;
    } else {
      // Smooth random walk within base ± variance
      v += (Math.random() - 0.5) * cfg.variance * 0.25;
      v = parseFloat(
        Math.max(cfg.base - cfg.variance, Math.min(cfg.base + cfg.variance, v)).toFixed(1)
      );
    }
    readings.push({
      id: count - i + 1,
 ...
 */
export async function getHistoryReadings(sensorType, intervalMinutes = 1, count = 60) {
